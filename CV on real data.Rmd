...
for (j in test_indecies){
  ...
  if (prior_str == "PC"){
    formula_ICAR <- Y ~ offset(log(E)) + 
      ...
      hyper = list(prec = list(prior = "pc.prec", param = c(3, 0.05))))
...
Failed on the sixth disease with gamma prior, should mention that the valdation sometimes chrashes, but not consistently.
...
summary(res_ICAR)
summary(res_ICAR_2)
#also no difference, so then the log = true should be removed
```
...
#saveRDS(criteria_df, file = "CV-Results//CV_no_iidTheta_gamma_W_C_final_final.rds")

#saveRDS(criteria_df, file = "CV-Results//CV_no_iidTheta_PC_W_C_final_final.rds")

#saveRDS(criteria_df, file = "CV-Results//CV_no_iidTheta_U_W_C_final_final.rds")
```
...
res_df <- readRDS(file = "CV-Results//final with correct BW prior//CV_no_iidTheta_gamma_W_C_final_final.rds")
prior <- "gamma"

#res_df <- readRDS(file = "CV-Results//final with correct BW prior//CV_no_iidTheta_PC_W_C_final_final.rds")
#prior <- "PC"

#res_df <- readRDS(file = "CV-Results//final with correct BW prior//CV_no_iidTheta_U_W_C_final_final.rds")
#prior <- "U"
