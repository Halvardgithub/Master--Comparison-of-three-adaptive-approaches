...
ABYM20_model <- inla.rgeneric.define(inla.rgeneric.Adaptive.ICAR, Q = Q_ABYM20)
ABYM50_model <- inla.rgeneric.define(inla.rgeneric.Adaptive.ICAR, Q = Q_ABYM50)
ABYM86_model <- inla.rgeneric.define(inla.rgeneric.Adaptive.ICAR, Q = Q_ABYM86)

NSRW10_model <- inla.rgeneric.define(inla.rgeneric.Adaptive.ICAR, Q = Q_NSRW10)
NSRW20_model <- inla.rgeneric.define(inla.rgeneric.Adaptive.ICAR, Q = Q_NSRW20)
NSRW50_model <- inla.rgeneric.define(inla.rgeneric.Adaptive.ICAR, Q = Q_NSRW50)
NSRW86_model <- inla.rgeneric.define(inla.rgeneric.Adaptive.ICAR, Q = Q_NSRW86)
```
...
test_indecies <- c(1, 10, 86) #some numbers for the diseases to be validated on
...
for (j in test_indecies){
  ...
  formula_RW_10 <- Y ~ log(E) + f(ID, model = ABYM10_model) #intercept is automatically included
  formula_RW_20 <- Y ~ log(E) + f(ID, model = ABYM20_model)
  formula_RW_50 <- Y ~ log(E) + f(ID, model = ABYM50_model)
  formula_RW_86 <- Y ~ log(E) + f(ID, model = ABYM86_model)
  ...
  formula_EW_10 <- Y ~ log(E) + f(ID, model = NSRW10_model)
  formula_EW_20 <- Y ~ log(E) + f(ID, model = NSRW20_model)
  formula_EW_50 <- Y ~ log(E) + f(ID, model = NSRW50_model)
  formula_EW_86 <- Y ~ log(E) + f(ID, model = NSRW86_model)
  ...
  vec_waic_dic <- c(res_RW_10$waic$waic, res_RW_20$waic$waic, res_RW_50$waic$waic,
                    ...
                    res_EW_86$dic$dic)
  vec_cpo <- c(-res_RW_10$cpo$logcpo/nAreas, -res_RW_20$cpo$logcpo/nAreas,
               -res_RW_50$cpo$logcpo/nAreas, -res_RW_86$cpo$logcpo/nAreas,
               -res_EW_10$cpo$logcpo/nAreas, -res_EW_20$cpo$logcpo/nAreas,
               -res_EW_50$cpo$logcpo/nAreas, -res_EW_86$cpo$logcpo/nAreas)
  #-mean(log(res_RW1$cpo$cpo))
  vec_BW <- c(res_BW$waic$waic, res_BW$dic$dic, -res_BW$cpo$logcpo/nAreas)
  
  criteria_df[j] <- c(vec_waic_dic, vec_cpo, vec_BW)
}
