...
test_indecies <- 1:5 #some numbers for the diseases to be validated on
...
for (j in test_indecies){
  ...
  formula_BW_fit <- Y ~ offset(log(E)) + f(ID, model = Wakefield_model)
  res_BW_fit <- inla(formula_BW_fit, family = "poisson", data = data_df)
  
  #construct BW Q from tau1 and tau2
  #summary(res_BW_fit)
  taus <- c(res_BW_fit$summary.hyperpar[, "mean"])
  Q_mat <- taus[1]*R1 + taus[2]*R2
  BW_model <- inla.rgeneric.define(inla.rgeneric.Adaptive.ICAR, R = Q_mat, nAreas = nAreas)
