...
#test_indecies <- 7:33 #indxes for cancers
#colnames(ObservedCases)[7:33]
...
#saveRDS(criteria_df, file = "CV-Results//CV_no_iidTheta_PC.rds")
```
...
WAIC_boxplot <- ggplot(WAIC_long, aes(x = Model, y = WAIC)) +
  ...
  #geom_hline(yintercept = mean(WAIC_long$WAIC, na.rm = TRUE), 
...
DIC_boxplot <- ggplot(DIC_long, aes(x = Model, y = DIC)) +
  geom_boxplot(outlier.shape = NA, fill = "skyblue") +
  #geom_hline(yintercept = mean(DIC_long$DIC, na.rm = TRUE), 
   #          linetype = "dashed", color = "red", linewidth = 1) +
  theme_minimal() +
  labs(x = "", y = "DIC") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
CPO_boxplot <- ggplot(CPO_long, aes(x = Model, y = CPO)) +
  geom_boxplot(outlier.shape = NA, fill = "skyblue") +
  theme_minimal() +
  #geom_hline(yintercept = mean(CPO_long$CPO, na.rm = TRUE), 
   #          linetype = "dashed", color = "red", linewidth = 1) +
  labs(x = "", y = "LS") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  coord_cartesian(ylim = c(lower, upper))
...
```{r}
#ggsave("Plots//Plots_withScaling_and_without_iidTheta//CV_boxplot_ICAR_ref_WO_outliers.png", plot = full_boxplot, height = 24, width = 21,  units = "cm",  dpi = 300)

#ggsave("Plots//Plots_withScaling_and_without_iidTheta//CV_boxplot_ICAR_ref_WO_outliers_only_cancer.png", plot = full_boxplot, height = 24, width = 21,  units = "cm",  dpi = 300)

## Figures for the powerpoint:
#ggsave("Plots//CV_plot_DIC_cancer_example.png", plot = DIC_boxplot, height = 9, width = 17,  units = "cm",  dpi = 300)

#ggsave("Plots//RW_edgeplot_example.png", plot = edgeplot_ABYM_86, height = 16, width = 15,  units = "cm",  dpi = 300)
```
