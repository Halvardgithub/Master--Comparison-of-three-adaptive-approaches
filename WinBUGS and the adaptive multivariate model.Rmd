...
get_causes <- function(n){ #nDiseasesTot is defined further up
  #set.seed(12345) # the main seed
  set.seed(44) # the secondary seed for comparisons
...
RW_ICAR_withIID <- function() {
  ...
  for (i in 1:Nareas) {
    for (m in 1:Ndiseases) {
      ...
      phi[i, m] ~ dnorm(mean.phi[i, m], prec.phi[i, m]) #uses precision
      # Prior distribution for non-spatial effects
      theta[i, m] ~ dnorm(0, 1) #uses precision
    }
  }
  for (i in 1:n.adj) {
  ...
  # Precision for the conditional distribution of spatial effects
  for (m in 1:Ndiseases) {
  ...
  # Mean for the conditional distribution of spatial effects
  for (m in 1:Ndiseases) {
    ...
    ceros[m] <- 0
    ceros[m] ~ dnorm(sum.phi[m], 10) #uses precision
    sum.phi[m] <- sum(phi[, m])
  }
...
```{r}
res_list_RW <- readRDS("Results//Results_withScaling_and_without_iidTheta//result_list_RW.rds") #with the main seed
#res_list_RW <- readRDS("Results//Results_for_second_seed//result_list_RW.rds") #with the second seed 
...
range(res_list_RW[[1]]$summary[, "n.eff"])
range(res_list_RW[[2]]$summary[, "n.eff"])
range(res_list_RW[[3]]$summary[, "n.eff"])
range(res_list_RW[[4]]$summary[, "n.eff"])
range(res_list_RW[[6]]$summary[, "n.eff"])
```
...
EW_ICAR_withIID <- function() {
  ...
  #to avoid nesting too many lists inside each other, caused crashes
  for(i in 1:n.adj){
  ...
  # Prior distributions for Tau2 - all the precision edges
  for (t in 1:(n.adj/2)){
...
EW_ICAR_NOiid <- function() {
  ...
  # Prior distributions for Tau2 - all the edge precisions
  for (t in 1:(n.adj/2)){
    Tau2[t] ~ dgamma(alpha, alpha) %_% I(0.001, )
  }
  alpha <- pow(sd.alpha, -2)
...
```{r}
res_list_EW <- readRDS("Results//Results_withScaling_and_without_iidTheta//result_list_EW.rds") #with the main seed
#res_list_EW <- readRDS("Results//Results_for_second_seed//result_list_EW.rds") #with the second seed
...
#check n.eff/ESS [, "n.eff"]
range(res_list_EW[[1]]$summary[, "n.eff"])
range(res_list_EW[[2]]$summary[, "n.eff"])
range(res_list_EW[[3]]$summary[, "n.eff"])
range(res_list_EW[[4]]$summary[, "n.eff"])
range(res_list_EW[[6]]$summary[, "n.eff"])

#Note that the number of parameters in each model also varies as it is tied to the number of diseases
length(which(res_list_EW[[1]]$summary[, "n.eff"] < 500))
length(which(res_list_EW[[2]]$summary[, "n.eff"] < 500))
length(which(res_list_EW[[3]]$summary[, "n.eff"] < 500))
length(which(res_list_EW[[4]]$summary[, "n.eff"] < 500))
length(which(res_list_EW[[6]]$summary[, "n.eff"] < 500))

length(which(res_list_EW[[1]]$summary[, "n.eff"] < 5000))
length(which(res_list_EW[[2]]$summary[, "n.eff"] < 5000))
length(which(res_list_EW[[3]]$summary[, "n.eff"] < 5000))
length(which(res_list_EW[[4]]$summary[, "n.eff"] < 5000))
length(which(res_list_EW[[6]]$summary[, "n.eff"] < 5000))
```
...
# Loop through each region to create edges and store region IDs
...
xtable(anova_log_df, digits = 3)

anova_log_edge_table <- ggtexttable(anova_log_df, theme = ttheme("mOrange"))

#ggsave("Plots//Plots_withScaling_and_without_iidTheta_withCancer//anovaLogFigure.png", plot = anova_log_edge_table, height = 4, width = 6,  units = "cm",  dpi = 300)
```
...
# Table for the region specific weights c in the RW-ICAR
```{r}

#retrieve the weights c
df_weights_c <- data.frame(res_list_RW[[1]]$mean$c, res_list_RW[[2]]$mean$c,
                           res_list_RW[[3]]$mean$c, res_list_RW[[4]]$mean$c,
                           res_list_RW[[6]]$mean$c)

#retrieve the standard deciation for each c
df_weights_c_sd <- data.frame(
  res_list_RW[[1]]$summary[grep("^c\\[", rownames(res_list_RW[[1]]$summary)), "sd"],
  res_list_RW[[2]]$summary[grep("^c\\[", rownames(res_list_RW[[2]]$summary)), "sd"],
  res_list_RW[[3]]$summary[grep("^c\\[", rownames(res_list_RW[[3]]$summary)), "sd"],
  res_list_RW[[4]]$summary[grep("^c\\[", rownames(res_list_RW[[4]]$summary)), "sd"],
  res_list_RW[[6]]$summary[grep("^c\\[", rownames(res_list_RW[[6]]$summary)), "sd"])

formatted <- matrix(nrow = nrow(df_weights_c), ncol = ncol(df_weights_c))

for (i in 1:nrow(df_weights_c)) {
  for (j in 1:ncol(df_weights_c)) {
    formatted[i, j] <- sprintf("%.2f (%.2f)", df_weights_c[i, j], df_weights_c_sd[i, j])
  }
}

formatted_df <- as.data.frame(formatted)

rownames(formatted_df) <- ss_provinces$Texto
colnames(formatted_df) <- c("n=10", "n=20", "n=50", "n=86", "Cancer")
