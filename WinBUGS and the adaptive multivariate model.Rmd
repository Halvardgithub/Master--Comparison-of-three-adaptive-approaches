...
NSRW1_model <- function() {
  ...
  for (t in 1:length(Tau2)){
    Tau2[t] ~ dgamma(1, 1.2) #or something like that, could also have hyperparameters and priors
  }
  # Other prior distributions
  for (j in 1:Ndiseases) {
    sd.phi[j] ~ dunif(0, 5)
...
data <- list(O = ObservedCases, E = ExpectedCases,
             Nareas = nrow(ObservedCases), Ndiseases = length(causes.id),
             n.adj = length(carto.wb$adj), adj = carto.wb$adj, index = index, tau.list = my_list)
# Initial values
...
# Variables to retrieve
param_NSRW1 <- c("mu", "lambda", "sd.phi", "phi", "sd.theta", "theta","SMR", "Tau2")
# Calls to WinBUGS
