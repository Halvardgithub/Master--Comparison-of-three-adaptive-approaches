...
# Calls to WinBUGS
results.AdaptiveBYM.cancer <- pbugs(data = data, inits = initials,
                              parameters.to.save = param, model = AdaptiveBYM_model_NOiid,
...
result_list_ABYM <- list(results.AdaptiveBYM.10, results.AdaptiveBYM.20, 
                         ...
                         runtime_ABYM, results.AdaptiveBYM.cancer)
saveRDS(result_list_ABYM, file = "Results//result_list_ABYM.rds") #move it to correct folder afterwards
```
...
res_list_ABYM[[5]] #check runtimes
...
range(res_list_ABYM[[6]]$summary[, "Rhat"])
...
which(res_list_ABYM[[6]]$summary[, "Rhat"] > 1.1)
...
# Calls to WinBUGS
results.NSRW1.cancer <- pbugs(data = data, inits = initials_NSRW1,
                              parameters.to.save = param_NSRW1, model = NSRW1_model_NOiid,
                              n.iter = 25000, n.burnin = 10000, n.chains = 3, DIC = F,
                              debug = F)
t1 <- Sys.time()
...
result_list_NSRW1 <- list(results.NSRW1.10, results.NSRW1.20, results.NSRW1.50, 
                    results.NSRW1.86, runtime_NSRW1, results.NSRW1.cancer)
...
range(res_list_NSRW1[[6]]$summary[, "Rhat"])
...
which(res_list_NSRW1[[6]]$summary[, "Rhat"] > 1.1)
...
res_list_NSRW1[[6]]$summary["alpha",]
...
edges_sf$edge_value_NSRW1_cancer <- res_list_NSRW1[[6]]$mean$Tau2[my_list]*
      mean(1/(res_list_NSRW1[[6]]$mean$sd.phi)^2)
...
# all cancers
count_num <- 1
tau_list_ABYM_cancer <- rep(0, sum(carto.wb$num))
for (i in 1:nAreas){
  for (j in carto.nb[[i]]){
    tau_list_ABYM_cancer[count_num] <- sqrt(res_list_ABYM[[6]]$mean$c[i]*
            res_list_ABYM[[6]]$mean$c[j])*mean(1/(res_list_ABYM[[6]]$mean$sd.phi)^2)
    count_num <- count_num + 1
  }
}
...
edges_sf$edge_value_ABYM_cancer <- tau_list_ABYM_cancer
...
df_corr_NSRW1 <- round(cor(cbind(edges_sf$edge_value_NSRW1_10, edges_sf$edge_value_NSRW1_20,
                                 edges_sf$edge_value_NSRW1_50, edges_sf$edge_value_NSRW1_86)),
                           4)
                                 
                            

df_corr_NSRW1 <- as.matrix(df_corr_NSRW1)

rownames(df_corr_NSRW1) <- c("n=10", "n=20", "n=50", "n=86")
colnames(df_corr_NSRW1) <- c("n=10", "n=20", "n=50", "n=86")

df_corr_tbl <- as.data.frame(df_corr_NSRW1)
...
## Then for RW-ICAR
```{r}
df_corr_ABYM <- round(cor(cbind(edges_sf$edge_value_ABYM_10, edges_sf$edge_value_ABYM_20,
                                 edges_sf$edge_value_ABYM_50, edges_sf$edge_value_ABYM_86)),
                           4)

df_corr_ABYM <- as.matrix(df_corr_ABYM)

rownames(df_corr_ABYM) <- c("n=10", "n=20", "n=50", "n=86")
colnames(df_corr_ABYM) <- c("n=10", "n=20", "n=50", "n=86")

df_corr_tbl <- as.data.frame(df_corr_ABYM)
df_corr_tbl <- tibble::rownames_to_column(df_corr_tbl, var = "Sample size")
