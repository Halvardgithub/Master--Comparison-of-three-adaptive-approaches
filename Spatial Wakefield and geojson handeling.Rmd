...
SpainMap_W_graph <- ggplot() +
  ...
  geom_sf(data = edges_sf, linewidth = 0.8, color = "blue") + 
  theme_minimal()
...
SpainMap_Wakefieldgraph <- ggplot() +
  ...
  geom_sf(data = ss_ar, color="black", fill=NA,linewidth = 0.4) + # Autonomous regions
  geom_sf(data = centroids, color = "red", size = 3) +  # Centroids
  geom_sf(data = edges_sf, aes(linewidth = inter_region_edges), color = "blue") + 
...
ExamplePlot1 <- ggplot() +
  ...
  geom_sf(data=ss_provinces_example, color="grey") +
  geom_sf(data = ss_ar_example, color="black", fill=NA,linewidth = 0.4) +
  geom_sf(data = centroids_example, color = "red", size = 3)  + # Centroids
  geom_sf(data = edges_sf_example, aes(color = Wex21), show.legend = FALSE) +
  scale_color_manual(values = c("0" = "blue", "1" = "red")) +
...
inla.rgeneric.AdaptiveICAR = function(
  ...
  theta = NULL)
{
  #Input: (prior_str currently not passed, just using pc)
  ...
  # AMat is the adjacency matrix, so simply 1 for bordering regions, an ICAR structure
